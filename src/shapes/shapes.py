...
def cycle_3(start, len_cycle, role_start=0, plot=False):

    graph = nx.Graph()
    graph.add_nodes_from(range(start, start + len_cycle))
    for i in range(len_cycle-1):
        graph.add_weighted_edges_from([(start + i, start + i + 1,1)])
    graph.add_weighted_edges_from([(start + len_cycle - 1, start,1)])
    roles = [1,2,3] * int(len_cycle/3)
    if plot is True: plot_networkx(graph, roles)
    return graph, roles


def cycle_6(start, len_cycle, role_start=0, plot=False):
    ...
    for i in range(len_cycle-1):
        graph.add_weighted_edges_from([(start + i, start + i + 1,1)])
    graph.add_weighted_edges_from([(start + len_cycle - 1, start,1)])
    roles = [1,2,3,4,5,6] * int(len_cycle/6)
    if plot is True: plot_networkx(graph, roles)
    return graph, roles
...
def fan1(start, nb_branches, role_start=0, plot=False):

    graph, roles = star1(start, nb_branches, role_start=role_start)
    for k in range(1, nb_branches - 1):
        roles[k] += 1
        roles[k + 1] += 1
        graph.add_weighted_edges_from([(start + k, start + k + 1,3)])
    if plot is True: plot_networkx(graph, roles)
    return graph, roles

def fan2(start, nb_branches, role_start=0, plot=False):
   
    graph, roles = star2(start, nb_branches, role_start=role_start)
    for k in range(1, nb_branches - 1):
        roles[k] += 1
        roles[k + 1] += 1
        graph.add_weighted_edges_from([(start + k, start + k + 1,2)])
    if plot is True: plot_networkx(graph, roles)
    return graph, roles
...
def house1(start, role_start=0, plot=False):
   
    graph = nx.Graph()
    graph.add_nodes_from(range(start, start + 5))
    graph.add_weighted_edges_from([(start, start + 1,3), (start + 1, start + 2,3),
                          (start + 2, start + 3,3), (start + 3, start,3)])
    graph.add_weighted_edges_from([(start, start + 2,3), (start + 1, start + 3,3)])
    graph.add_weighted_edges_from([(start + 4, start,3), (start + 4, start + 1,3)])
    roles = [role_start, role_start, role_start + 1,
             role_start + 1, role_start + 2]
    if plot is True: plot_networkx(graph, roles)
    return graph, roles

def house2(start, role_start=0, plot=False):
    
    graph = nx.Graph()
    graph.add_nodes_from(range(start, start + 5))
    graph.add_weighted_edges_from([(start, start + 1,2), (start + 1, start + 2,2),
                          (start + 2, start + 3,2), (start + 3, start,2)])
    graph.add_weighted_edges_from([(start, start + 2,2), (start + 1, start + 3,2)])
    graph.add_weighted_edges_from([(start + 4, start,2), (start + 4, start + 1,2)])
    roles = [role_start, role_start, role_start + 1,
             role_start + 1, role_start + 2]
    if plot is True: plot_networkx(graph, roles)
    return graph, roles
...
def star1(start, nb_branches, role_start=0, plot=False):
    
    graph = nx.Graph()
    graph.add_nodes_from(range(start, start + nb_branches + 1))
    for k in range(1, nb_branches + 1):
        graph.add_weighted_edges_from([(start, start + k,3)])
...
def star2(start, nb_branches, role_start=0, plot=False):
    
    graph = nx.Graph()
    graph.add_nodes_from(range(start, start + nb_branches + 1))
    for k in range(1, nb_branches + 1):
        graph.add_weighted_edges_from([(start, start + k,2)])
    roles = [role_start + 1] * (nb_branches + 1)
    roles[0] = role_start
    if plot is True: plot_networkx(graph, roles)
    return graph, roles
