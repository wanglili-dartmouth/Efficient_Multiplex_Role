...
def main(args):
    ...
    basis_type = "cycle_6" 
    nb_shapes = 6 
    list_shapes = [["fan1",6],["star1",6],["house1"],["fan2",6],["star2",6],["house2"]]*3
    sample = open('varied_p.out', 'w') 
    Total_times=10
    ami=[]
    sil=[]
    hom=[]
    ...
    for times in range(Total_times):
        G, communities, plugins, role_id = build_structure(width_basis, basis_type, list_shapes, start=0,
                            rdm_basis_plugins =False, add_random_edges=18,
        ...
        nx.write_weighted_edgelist(G, "test2.edgelist")

        graph = network.read_from_edgelist("test2.edgelist", index_from_zero=args.index_from_0)
